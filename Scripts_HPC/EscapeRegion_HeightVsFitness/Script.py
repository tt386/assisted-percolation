...
fitness = DataMatrix[ParamPairID]

print("fitness",fitness)

SpecificSaveFile = str(SaveFile) + '/fitness_%0.3f'%(fitness)
...
#Determine the location of the middle bottom site
X = int(Centerposx)
SeededPos = []
    ...
    if (Centerposx - X)**2 + 0.75*(Centerposy - Y) **2 >= Radius**2:
        ...
        for n in NeighbourList:
            ...
            if (((Centerposx - x)**2 + 0.75*(Centerposy - y)**2) <
                    Radius**2):
                if len(SeededPos) == 0:#[X,Y] not in SeededPos:
                    SeededPos = tuple([X,Y])
