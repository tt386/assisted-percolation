...
for j in range(-rows,rows):
    midy = j * centersep*np.sin(np.pi/3)
    for i in range(-columns,columns):
        if j%2 == 0:
            midx = i * centersep
        else:
            midx = i * centersep + centersep*np.cos(np.pi/3)
    
        CenterList.append((midx,midy))

for i in CenterList:
    ax.add_patch(plt.Circle((i[0],i[1]),r, facecolor='w',edgecolor='k',linewidth=2))
...
plt.xlim(0,10)#columns*centersep)
plt.ylim(0,10)
...
plt.savefig("Horizontal.eps",bbox_inches='tight',pad_inches=0.0)#,bbox_inches=bbox)
plt.close()
...
for j in range(-rows,rows):
    midy = j * centersep*np.sin(np.pi/6)
    for i in range(-columns,columns):
        if j%2 == 0:
            midx = i * centersep * np.cos(np.pi/6) * 2
        else:
            midx = i * centersep * 2*np.cos(np.pi/6) + centersep * np.cos(np.pi/6)
...
plt.xlim(0,10)#columns*centersep*2)
plt.ylim(0,10)#rows*centersep*np.sin(np.pi/6))
...
plt.savefig("Vertical.eps",bbox_inches='tight',pad_inches=0.0)#,bbox_inches=bbox)
"""
mid = r + s/2
