...
plt.plot([np.cos(np.pi/8),0],[np.sin(np.pi/8),H],zorder=5,color = red,linewidth=3)

#The WT Path
WTPathx = []
WTPathy = []

thetalist = np.linspace(np.pi/8,np.pi/2-np.arccos(1/H),1000)
...
OCx = [0,np.cos(np.pi/2-np.arccos(1/H))]
OCy = [0,np.sin(np.pi/2-np.arccos(1/H))]
...
OIx = [0,np.cos(np.pi/8)]
OIy = [0,np.sin(np.pi/8)]
plt.plot(OIx,OIy,'--k',linewidth=3)


#Poor man's attempt to keep scale of Fig5 the same
plt.scatter([0,0.1],[2,2],s=0,color='white')
...
plt.text(-0.15,H,"D",fontsize=fs,ha='center', va='center')

plt.text(np.cos(np.pi/2-np.arccos(1/H))+0.1,np.sin(np.pi/2-np.arccos(1/H))+0.1,
    "C",fontsize=fs,ha='center', va='center')

plt.text(1.15,0,"B",fontsize=fs,ha='center', va='center')

plt.text(np.cos(np.pi/8)+0.2,np.sin(np.pi/8),
...
"""
plt.text(0.075,0.3,r"$\gamma$",fontsize=fs,ha='center', va='center')

plt.text(0.3,0.25,r"$\psi$",fontsize=fs,ha='center', va='center')

plt.text(0.45,0.09,r"$\rho$",fontsize=fs,ha='center', va='center')
##################
#Formatting
plt.xlim(-1,1.2)
plt.ylim(-1,2.2)#H+0.2)
...
#bbox = Bbox([[-1.1,-1.1],[1.3,H+0.2]]) #[x0,y0],[x1,y1]
bbox = Bbox([[-1.1,-1.1],[1.3,2.3]])
