...
#right
rightcircle = plt.Circle((2*r+s,0),r, facecolor='w',edgecolor='k',linewidth=2)
ax.add_patch(rightcircle)

#middle
topx = (2*r+s)*np.cos(np.pi/3)
topy = (2*r+s)*np.sin(np.pi/3)
topcircle = plt.Circle((topx,topy),r, facecolor='w',edgecolor='k',linewidth=2)
ax.add_patch(topcircle)

#right middle
toprightcircle = plt.Circle((topx+2*r+s,topy),r, facecolor='w',edgecolor='k',linewidth=2)
ax.add_patch(toprightcircle)

#left middle
topleftcircle = plt.Circle((topx-(2*r+s),topy),r, facecolor='w',edgecolor='k',linewidth=2)
ax.add_patch(topleftcircle)

#above left
upperleftcircle = plt.Circle((0,2*topy),r,facecolor='w',edgecolor='k',linewidth=2)
ax.add_patch(upperleftcircle)

#Above right
upperrightcircle = plt.Circle((2*r+s,2*topy),r,facecolor='w',edgecolor='k',linewidth=2)
ax.add_patch(upperrightcircle)



#The Mutant path
plt.plot([mid+r,mid+r],[0,topy],color=red,linewidth=3)
...
for i in np.linspace(-tangentangle,0,1000):#np.pi/2-np.arccos(0.5),1000):
...
"""
plt.scatter([r*np.cos(tangentangle)],[r*np.sin(tangentangle)],color='k',s=m,zorder=3)
...
plt.scatter([topx - r*np.cos(tangentangle)],[topy - r*np.sin(tangentangle)],color='k',s=m,zorder=3)
...
"""
plt.scatter([topx-r*np.cos(np.pi/2-np.arccos(0.5))],[topy + r*np.sin(np.pi/2-np.arccos(0.5))],color='k',s=m,zorder=3)
plt.text(topx-r*np.cos(np.pi/2-np.arccos(0.5))-0.2,topy + r*np.sin(np.pi/2-np.arccos(0.5)),
    "E",fontsize=fs,ha='center', va='center')
"""

plt.scatter([topx+r],[0],color='k',s=m,zorder=3)
plt.text(topx+r-0.2,0,
    "J",fontsize=fs,ha='center', va='center')

plt.scatter([topx+r],[topy],color='k',s=m,zorder=3)
plt.text(topx+r-0.2,topy,
    "K",fontsize=fs,ha='center', va='center')
...
bbox = Bbox([[-0.2,-0.5],[3.1,3.2]]) #[x0,y0],[x1,y1]
...
plt.savefig("LongTerm.pdf",bbox_inches=bbox)
